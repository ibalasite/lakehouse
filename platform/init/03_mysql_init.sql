...
-- ── 4. Daily report cache (Lambda UNION ALL serving MV) ──────────────────────
-- Pre-joined flat table read by Metabase; combines batch (D-1) + intraday (D+0).
-- Partitioned by date_sk to match cache_ticket_daily rotation pattern (EDD §10.6.5).
